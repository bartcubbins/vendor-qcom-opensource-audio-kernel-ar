...
static void *def_wcd_mbhc_cal(void)
{
	...
	btn_high[0] = 75;
	btn_high[1] = IS_ENABLED(CONFIG_ARCH_SONY_NAGARA) ? 137 : 150;
	btn_high[2] = 237;
	btn_high[3] = 500;
	btn_high[4] = 500;
	btn_high[5] = 500;
...
static int cs35l41_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	for (i = rtd->dai_link->num_cpus; i < (rtd->dai_link->num_cpus + rtd->dai_link->num_codecs); i++) {
		...
		if (dapm->component->name_prefix == NULL) {
			...
			if (snd_soc_dapm_ignore_suspend(dapm, "AMP Playback") < 0)
				pr_err("%s: ignore suspend failed: AMP Playback\n", __func__);
			if (snd_soc_dapm_ignore_suspend(dapm, "AMP Capture") < 0)
				pr_err("%s: ignore suspend failed: AMP Capture\n", __func__);
			...
			if (snd_soc_dapm_ignore_suspend(dapm, "AMP Playback") < 0)
				pr_err("%s: ignore suspend failed: %s AMP Playback\n", __func__, dapm->component->name_prefix);
			if (snd_soc_dapm_ignore_suspend(dapm, "AMP Capture") < 0)
				pr_err("%s: ignore suspend failed: %s AMP Capture\n", __func__, dapm->component->name_prefix);
			if (snd_soc_dapm_ignore_suspend(dapm, "SPK") < 0)
